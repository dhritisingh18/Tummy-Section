...
export const getMenuItemsByRestaurantId = ( reqData ) => {
    return async (dispatch) => {
        console.log("ENTERED MENU ITEMS BY RES ID")
        dispatch({ type: GET_MENU_ITEMS_BY_RESTAURANT_ID_REQUEST })

        try {
            const { data } = await api.get(`api/food/get-restaurant-food/${reqData.restaurantId}?isVegetarian=${reqData.vegetarian}&isNonVeg=${reqData.nonVeg}&isSeasonal=${reqData.seasonal}&foodCategory=${reqData.foodCategory}`, {
                headers: {
...
export const updateMenuItemAvailability = ({foodId, jwt}) =>{
    ...
    return async (dispatch) =>{
        ...
        try{
            ...
            const {data} = await api.put(`api/admin/food/${foodId}/update-availability-status`,{},{
                headers: {
                    Authorization : `Bearer ${jwt}`
                }
            })
...
export const deleteFoodItem = ({foodId, jwt}) =>{
